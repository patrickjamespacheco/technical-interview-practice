...
def validate_guide(key: str, guide: Any, part_count: int) -> None:
    ...
    allowed = {"approach", "verify", "lesson"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.check:
            ...
            for directory in (SOURCE_DIR_NAME, LESSON_DIR_NAME):
            ...
            print(f"OK: {OUTPUT_NAME}, {len(outputs) - lesson_count - 1} source files, and {lesson_count} lesson files are current")
        else:
            for directory in (SOURCE_DIR_NAME, LESSON_DIR_NAME):
            ...
            lesson_count = sum(path.parent.name == LESSON_DIR_NAME for path in outputs)
            print(f"Wrote {OUTPUT_NAME}, {len(outputs) - lesson_count - 1} source files, and {lesson_count} lesson files")
        return 0
